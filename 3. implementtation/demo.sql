...
CREATE TABLE IF NOT EXISTS `ngoa`.`tasktype` (
  ...
  INDEX `fk_tasktype_task1_idx` (`task_idtask` ASC)  COMMENT '',
  CONSTRAINT `fk_tasktype_task1`
    FOREIGN KEY (`task_idtask`)
    REFERENCES `ngoa`.`task` (`idtask`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `ngoa`.`doctype` (
  ...
  INDEX `fk_doctype_document1_idx` (`document_iddocument` ASC)  COMMENT '',
  CONSTRAINT `fk_doctype_document1`
    FOREIGN KEY (`document_iddocument`)
    REFERENCES `ngoa`.`document` (`iddocument`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `ngoa`.`contacttype` (
  ...
  INDEX `fk_contacttype_contact1_idx` (`contact_idcontact` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `ngoa`.`notestype` (
  ...
  INDEX `fk_notestype_notes1_idx` (`notes_idnotes` ASC)  COMMENT '',
  CONSTRAINT `fk_notestype_notes1`
    FOREIGN KEY (`notes_idnotes`)
    REFERENCES `ngoa`.`notes` (`idnotes`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `ngoa`.`meetingstype` (
  ...
  INDEX `fk_meetingstype_meetings1_idx` (`meetings_idmeetings` ASC)  COMMENT '',
  CONSTRAINT `fk_meetingstype_meetings1`
    FOREIGN KEY (`meetings_idmeetings`)
    REFERENCES `ngoa`.`meetings` (`idmeetings`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `ngoa`.`activitytype` (
  ...
  `activities_idactivities` INT NOT NULL COMMENT '',
  PRIMARY KEY (`idactivitytype`)  COMMENT '',
  INDEX `fk_activitytype_activities1_idx` (`activities_idactivities` ASC)  COMMENT '',
  CONSTRAINT `fk_activitytype_activities1`
    FOREIGN KEY (`activities_idactivities`)
    REFERENCES `ngoa`.`activities` (`idactivities`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
